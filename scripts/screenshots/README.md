...
The images in `docs/images/` that the README and the Marketplace listing use are rendered here from
**synthetic data**. Nothing in `scenarios.js` or `tree.html` is a real machine, workspace, session,
account or person, and nothing in this folder reads `~/.claude` or `~/.vineyard`. Keep it that way:
